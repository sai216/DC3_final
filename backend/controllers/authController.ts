...
const extractPrivyEmail = (privyUser: any): string | null => {
  const directEmail = privyUser?.email?.address || privyUser?.email?.email || privyUser?.email;
  if (typeof directEmail === 'string' && directEmail.trim()) {
    return directEmail.toLowerCase();
  }

  const linkedAccounts = Array.isArray(privyUser?.linked_accounts) ? privyUser.linked_accounts : [];
  for (const account of linkedAccounts) {
    const type = account?.type;
    const email = account?.address || account?.email;
    if ((type === 'email' || type === 'google_oauth') && typeof email === 'string' && email.trim()) {
  ...
  async authenticateWithPrivy(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (!email) {
        res.status(400).json({
          success: false,
          error: 'User must have an email address linked in Privy',
        });
        return;
      }
